...
describe('safe_pay', () => {
    ...
    const createMint = async (connection: anchor.web3.Connection): Promise<anchor.web3.PublicKey> => {
        ...
        console.log(`[${tokenMint.publicKey}, ${tokenMint.secretKey}] Created new mint account at ${signature}`);
    ...
    const readAccount = async (accountPublicKey: anchor.web3.PublicKey, provider: anchor.Provider): Promise<[spl.AccountInfo, string]> => {
        ...
        if (accountInfo.amount) {
            ...
            console.log("else-accountInfo", accountInfo)
        }
    ...
    it('Log', async () => {
        ...
        console.log(provider.wallet.publicKey.toBytes())
    ...
    it('can initialize a safe payment by Alice', async () => {
        ...
        const tx1 = await program.methods.initializeNewGrant(pda.idx, pda.stateBump, pda.escrowBump, amount).accounts({
